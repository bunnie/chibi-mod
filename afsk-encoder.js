function FskEncoder(data, sampleRate) {
    ...
    this.samplesPerBit = this.sample_rate / this.baud_rate; // Not rounded! Floating point!
    var numBits = this.inputData.length * 8; // and add any overhead here...
...
FskEncoder.prototype = {
    ...
    modulate: function(outputData) {
	for(var i = 0; i < outputData.length; i++) {
	    this.state.baud_frac += this.state.baud_incr;
	    if( this.state.baud_frac >= 1) {
		this.state.baud_frac -= 1;
		if( this.state.bitpos == 0 ) {
		    if( this.state.datapos <= this.inputData.length ) {
			this.state.curbyte = this.inputData[this.state.datapos++];
			this.state.bitpos = 8;
		    } else {
			return;
		    }
...
function AfskEncoder(data, sampleRate, baud) {
    ...
    utf8data = data;  // we're now sending raw binary, not text
    //this.expandFlags(utf8data); // into this.symbolData + numResidueBits
    this.numResidueBits = 0;
    this.symbolData = utf8data;
...
AfskEncoder.prototype = {
	...
	for (var c = 0; c < utf8data.length; c++) {
	    ...
	    residueBits = residueBits | (utf8data[c] << numResidueBits);
	    numResidueBits += 8;
